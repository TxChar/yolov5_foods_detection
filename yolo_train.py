...
def train(hyp, opt, device, callbacks):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            box_loss, obj_loss, cls_loss = loss_items

            print(f"mloss: {mloss}")
            print(f"obj: {obj_loss}")
            print(f"cls: {cls_loss}")
            print(f"box: {box_loss}")
            print(f"schedule: {optimizer.param_groups[0]['lr']}")

            # step =  epoch * len(pbar) + i
            step =  epoch
            writer.add_scalar("Loss/Overall loss", sum(loss), step)
            writer.add_scalar("Loss/Objectness loss", obj_loss, step)
            writer.add_scalar("Loss/Classification loss", cls_loss, step)
            writer.add_scalar("Loss/Box regression loss", box_loss, step)
            writer.add_scalar("Loss/Scheduled Learning Rate", optimizer.param_groups[0]['lr'], step)
